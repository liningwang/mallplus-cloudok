...
export default {
	...
	methods: {
		...
		async loadData(type = 'add', loading) {
			...
			if (_this.cateId) {
				params = { pageNum: _this.pageNum, productCategoryId: _this.cateId };
				if (_this.keyword) {
					params = { pageNum: _this._pageNum, productCategoryId: _this.cateId, keyword: _this.keyword };
				}
			}
			if (_this.keyword) {
				params = { pageNum: _this.pageNum, keyword: _this.keyword };
			}
			console.log("goodList params " + JSON.stringify(params));
			...
			_this.loadingType = _this.goodsList.length > list.total ? 'nomore' : 'more';
			if (type === 'refresh') {
				if (loading == 1) {
					uni.hideLoading();
				} else {
