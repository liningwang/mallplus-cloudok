            ...
            if (option.groupActivityId) { // 团购
                ...
                if (option.groupType) { // 拼团
                    ...
                    if (option.type == 1) { // 详情
                        ...
                        let params = { goodsId: option.id, type: option.type };
        ...
        methods: {
            ...
            async submit() {
                ...
                if (this.groupType) {
                    ...
                    // detail
                    if (this.type == 1) {
                        let params = {basicGiftsVar:this.basicGiftsVar,orderType:1, type: 1, cartId: this.cartId, addressId: this.addressId, couponId: this.couponId,memberCouponId: this.memberCouponId, content: this.desc };
                        if (this.groupActivityId) {
                            params = {groupActivityId:this.groupActivityId,orderType:3, type: 1, cartId: this.cartId, addressId: this.addressId, couponId: this.couponId,memberCouponId: this.memberCouponId, content: this.desc };
                        }
                        data = await Api.apiCall('post', Api.order.bookOrder, params);
                    }
                    // 购物车
                    if (this.type == 2) {
                        let params = {orderType:1, type: 2, cartIds: this.cartIds, addressId: this.addressId, couponId: this.couponId, content: this.desc };
                        data = await Api.apiCall('post', Api.order.bookOrder, params);
                    }

                    // 秒杀
                    if (this.type == 6) {
                        let params = { type: 6, skillId: this.skillId, addressId: this.addressId, couponId: this.couponId, content: this.desc ,orderType:6};
                        data = await Api.apiCall('post', Api.order.bookOrder, params);
                    }
                }
                console.log(data);
